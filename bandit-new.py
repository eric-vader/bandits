...
class BinomialBandit(MultiArmedBandit):
    ...
    def reset(self):
        ...
        self.model = pm.Model()
        with self.model:
            self.bin = pm.Binomial('binomial', n=self.n*np.ones(self.k, dtype=np.int),
                                   p=self.action_values, shape=(self.k,), transform=None)
        if self.t is not None:
            self._samples = pm.draw(self.bin, draws=self.t).squeeze()
            self._cursor = 0
